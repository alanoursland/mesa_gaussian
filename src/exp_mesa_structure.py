...
Run as:
    python exp_mesa_linear.py
    python exp_mesa_linear.py --seed 42
    python exp_mesa_linear.py --p-init 1.0
"""
...
def run_experiment(
    ...
    p_init: float = 2.0,
    seed: int = 0,
    out_dir: str = "results/exp_mesa_linear",
):
    # Setup
    ...
    true_dist = -slack  # positive outside
    corr = np.corrcoef(internal_test, true_dist)[0, 1]
    ...
    # print()
    # if mesa_detected:
    #     print("✓ MESA STRUCTURE DETECTED")
    #     print(f"  Inside std:  {inside_std:.3f} (threshold < 1.0)")
    #     print(f"  Separation:  {separation:.2f} (threshold > 3.0)")
    # else:
    #     print("✗ NO CLEAR MESA STRUCTURE")
    #     if inside_std >= 1.0:
    #         print(f"  Inside std too high: {inside_std:.3f} (need < 1.0)")
    #     if separation <= 3.0:
    #         print(f"  Separation too low: {separation:.2f} (need > 3.0)")
...
def parse_args():
    ...
    p.add_argument("--out-dir", type=str, default="results/exp_mesa_linear")
    return p.parse_args()
