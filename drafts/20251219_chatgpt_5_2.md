...
The distance-based view of ReLU activations leads to a direct inversion of the standard interpretive narrative. Rather than treating activation magnitude as evidence for the presence of a feature, the mesa framework treats activation as evidence of deviation from an acceptable region.

In the conventional interpretation, a neuron “fires” when an input matches its preferred stimulus, and larger activations are taken to indicate stronger confidence that the feature is present. This view implicitly borrows intuition from normalized similarity measures, even though the architecture itself does not enforce such normalization.

Under the mesa interpretation, the situation is reversed. A ReLU unit is quiescent when the input lies within its prototype region, defined by a half-space or an intersection of half-spaces. Activation occurs only when the input violates a learned constraint, and the magnitude of activation measures how far the input lies outside that region. Zero activation is therefore not absence of information, but maximal conformity.

This inversion can be summarized succinctly:
...
## 7. Prototype as Region (Conceptual Core)

### 7.1 Why point prototypes fail for ReLU

A common way to interpret neural features is in terms of point prototypes: a neuron is assumed to represent a single “ideal” input, and activation measures similarity to that prototype. For ReLU networks, this interpretation is fundamentally ill-posed.

First, ReLU activations are unbounded above. For any neuron with output (h(\mathbf{x}) = \mathrm{ReLU}(\mathbf{w}^\top \mathbf{x} + b)), if there exists any direction (\mathbf{x}) such that (\mathbf{w}^\top \mathbf{x} > 0), then scaling (\mathbf{x}) by an arbitrary positive factor increases the activation without bound. As a result, there is no finite maximally activating input unless additional constraints or normalization are imposed. The notion of an “ideal” point that best activates the neuron is therefore undefined in the native geometry of the model.

Second, even when normalization techniques are applied, the maximally activating input is not a stable or intrinsic object. It depends on external constraints such as input norms, preprocessing choices, or regularization schemes, none of which are specified by the architecture itself. This makes point-prototype interpretations contingent rather than structural.

From the distance-based perspective, this pathology is expected. A ReLU unit does not define a bounded region of attraction around a point; it defines a half-space bounded by a hyperplane. There is no preferred location deep inside that half-space, only increasing deviation in one direction. Attempting to interpret such a function in terms of a point prototype amounts to asking for a center where none exists.

This limitation is not merely conceptual but practical. Techniques such as activation maximization or feature visualization often produce extreme, uninterpretable inputs precisely because the optimization problem they pose is unbounded [Erhan et al., 2009]. The mesa framework reframes this failure: the problem is not that the methods are poorly designed, but that point prototypes are the wrong geometric object for ReLU networks.

### 7.2 Mesa as prototype

Replacing point prototypes with mesas resolves the geometric pathologies discussed in Section 7.1. In the mesa framework, a prototype is not a single maximally activating input, but a region of acceptability: the set of inputs for which all relevant constraints are satisfied and deviation is zero.

For a single ReLU unit, this prototype region is a half-space. For multiple units combined through positive aggregation, it becomes a convex polytope. In both cases, the defining property of the prototype is not where activation is large, but where it is absent. All inputs inside the mesa are equally prototypical; none is preferred over another by the model.

Meaning, in this view, is encoded not by a central template but by boundaries. Each hyperplane defines a condition that separates acceptable from unacceptable inputs. The identity of a concept is therefore determined by the conjunction of constraints it satisfies, rather than by proximity to a single exemplar. This perspective aligns naturally with the observation that many categories admit wide internal variation while still having relatively crisp exclusion criteria.

Interpreting prototypes as regions also clarifies why ReLU networks generalize well despite operating in high-dimensional spaces. Rather than memorizing points, the network learns surfaces that carve out large volumes of valid inputs. Generalization corresponds to the stability of these surfaces under perturbation, not to the density of stored exemplars.

This regional notion of prototype resonates with earlier theories in cognitive science, where categories are characterized by graded membership and family resemblance rather than strict definitions [Rosch, 1975]. The mesa framework provides a concrete geometric realization of this idea within modern neural architectures, grounded in the algebra of ReLU activations rather than in metaphor.

7.3 Recognition by exclusion

If prototypes are regions rather than points, recognition becomes a process of exclusion rather than matching. An input is accepted as belonging to a class not because it strongly activates a template, but because it survives a collection of constraints that eliminate alternatives.

In the mesa framework, each class is associated with a polyhedral region defined by the intersection of half-space constraints. Inputs inside this region incur little or no deviation; inputs outside it violate one or more constraints. Classification can therefore be understood as asking which class’s constraints are least violated, rather than which class’s template is most closely matched.

This view has two immediate consequences. First, it naturally supports rejection. If an input violates the constraints of all classes beyond some threshold, it can be identified as out-of-distribution without being forced into an arbitrary category. Second, it reframes ambiguity: inputs near shared boundaries may partially violate multiple sets of constraints, leading to graded or uncertain classification rather than confident but brittle decisions.

These properties connect directly to open-set recognition and anomaly detection. In open-set recognition, the goal is not only to classify known categories but also to detect when an input does not belong to any of them [Bendale and Boult, 2016]. Distance-based and energy-based methods have repeatedly been shown to outperform purely discriminative softmax models in this setting. The mesa framework provides a geometric explanation for this empirical success: open space is not tiled by point prototypes but bounded by constraint-defined regions with explicit margins.

Similarly, anomaly detection can be viewed as measuring distance from a learned prototype region rather than deviation from a single mean. Inputs that violate many constraints, or violate a few constraints by a large margin, naturally receive high energy under the polyhedral mesa model. This interpretation aligns ReLU networks with classical distance-based anomaly detection techniques while extending them to hierarchical, learned representations.
...
* Metric vs pseudometric discussion
* Relationship to Mahalanobis distance terminology
